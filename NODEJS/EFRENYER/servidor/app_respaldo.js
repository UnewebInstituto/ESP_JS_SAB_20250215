...
app.post('/login', async(req,res) =>{
    ...
    try {
        ...
        res.status(500).json({message:'Error en el servidor.', error:{code:500, detail:'Credenciales incorrectas'} });
        console.log(' ');
...
app.post('/usuario', async(req,res)=>{
  /*creacion de usuario o registro*/

  const{cedula, nombre, apellido, correo_electronico, clave, tipo_usuario} = req.body;

  try{
    //veririfcar a tgraves del colrreo, si el usuario se encuentra
    ...
    res.status(200).json({message: 'nuevo usuario fue registrado de forma correcta', error:{code:401}})
