...
function calcular(arg_a, arg_b, arg_c){
    // Se accede a los elementos <input> en el DOM y se convierte a valor numerico de tipo decimal
    ...
    */
    ...
    if (a==0){
        mensaje = "ERROR: El valor de 'a' deber ser diferente de 0";
        objetoSalida.className = "mensaje error";
    }
    else{
        subRadical = Math.pow(b,2) - 4 * a * c;
        if(subRadical < 0){
            mensaje = "ERROR: El resultado de la expresion sub radical deber ser mayor o igual a 0";
...
var a, b, b, subRadical, x1, x2, mensaje, objetoSalida;
